...
for t = 1:t_max
  ...
  H_VQ_over_time(t) = H_VQ;
  
  % normalise H_VQ
  H_VQ_over_time(t) = H_VQ_over_time(t)/P;
end
...
legend_plots = zeros(2*K, 1);
legend_plots(1) = plot(nan, nan, 'ro', 'MarkerFaceColor', colour_init_1);
legend_plots(2) = plot(nan, nan, 'ro', 'MarkerFaceColor', red);
legend_plots(3) = plot(nan, nan, 'go', 'MarkerFaceColor', colour_init_2);
legend_plots(4) = plot(nan, nan, 'go', 'MarkerFaceColor', green);
if K == 2
  legend(legend_plots, 'Cluster 1 Init', 'Cluster 1', ...
      'Cluster 2 Init', 'Cluster 2');
else
  legend_plots(5) = plot(nan, nan, 'bo', 'MarkerFaceColor', colour_init_3);
  legend_plots(6) = plot(nan, nan, 'bo', 'MarkerFaceColor', blue);
  legend_plots(7) = plot(nan, nan, 'mo', 'MarkerFaceColor', colour_init_4);
  legend_plots(8) = plot(nan, nan, 'mo', 'MarkerFaceColor', magenta);
  legend(legend_plots, 'Cluster 1 Init', 'Cluster 1', ...
      'Cluster 2 Init', 'Cluster 2', 'Cluster 3 Init', 'Cluster 3', ...
      'Cluster 4 Init', 'Cluster 4');
end


% plot quantisation error over time
figure; hold on;
plot(1:t_max, H_VQ_over_time, 'k-');
